...
1.2) Verify that the pods in the `metallb-system` are running.
...
[source,bash]
----
[user@host kbe]$ kubectl apply -f istio-${ISTIO_VERSION}/samples/helloworld/helloworld.yaml
...
[source,bash]
----
[user@host kbe]$ kubectl apply -f istio-${ISTIO_VERSION}/samples/helloworld/helloworld-gateway.yaml
...
////
5.8) Verify the parameters of the virtual service resource.

[source,bash]
----
[user@host kbe]$ kubectl get virtualservice helloworld -o yaml
  ...
  - match:
    ...
    - destination:
        ...
        port:
          number: 5000  # <5>
----
<1> The traffic will be handled by the `helloworld-gateway`.
<2> The ingress traffic will match requests with any host HTTP header.
<3> The requests will match if the `/hello` prefix is used.
<4> The destination service is set to `helloworld`.
<5> The destination port number is `5000`.
////
...
6.2) Check the commands contained in the `loadgen.sh` script.
